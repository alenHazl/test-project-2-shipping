...
`;
  ...
  async function initLocationAutocomplete() {
    ...
    function initLocationField(input, autocompleteService2, placesService, PlacesServiceStatus2) {
      ...
      function fetchCitySuggestions(query) {
        ...
        autocompleteService2.getPlacePredictions(request, async (predictions, status) => {
          if (status !== PlacesServiceStatus2.OK || !predictions) {
            dropdown.hide();
            return;
          }
      ...
      function getCityCountryFromPlaceId(placeId) {
        return new Promise((resolve) => {
          placesService.getDetails({ placeId, fields: ["address_components"] }, (result, status) => {
            if (status !== PlacesServiceStatus2.OK || !result?.address_components) {
              resolve(null);
              return;
            }
